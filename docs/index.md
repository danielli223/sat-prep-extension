...
| [`specs/2026-06-20-morph-check-to-explain.md`](specs/2026-06-20-morph-check-to-explain.md) | Decision (issue #27): after Check, morph the action button into **"Explain"** to free bottom space. "Explain" is a relabel of the existing Reveal control (un-hides CB's **own** native rationale) — **reaffirms** invariant #3 (no AI on CB content): nothing is synthesized. |
| [`plans/2026-06-20-morph-check-to-explain.md`](plans/2026-06-20-morph-check-to-explain.md) | Implementation plan for the Check→Explain morph: failing tests first (morph fires in `renderVerdict`, not on the needs-answer/stale paths), then fold the hide-Check + relabel-reveal into `renderVerdict`. No `content.ts`/`cb/`/store changes. |
