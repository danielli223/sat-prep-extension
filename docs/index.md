...
| [`specs/2026-06-20-student-bank-spa-login.md`](specs/2026-06-20-student-bank-spa-login.md) | Design (issue #70): fix the post-login gap where the student bank SPA-routes in without re-injecting. Broaden the match to `mypractice.collegeboard.org/*` **and** make the boot path-aware — a host-keyed `isQuestionBankPage` predicate (educator host always; student host only `/questionbank/*`) gates an idempotent `activate()`/`teardown()`, re-evaluated on `pushState`/`popstate`. Preserves educator behavior. |
| [`plans/2026-06-20-student-bank-spa-login.md`](plans/2026-06-20-student-bank-spa-login.md) | Implementation plan (issue #70): broaden 3 manifests + `packaging.test` allowlist; predicate + boot-lifecycle (`activate`/`teardown` + SPA-route listener) in `content.ts`; live `/verify-overlay` (login→SPA, UI off `/dashboard`, no educator regression). Stacks on PR #53. |
