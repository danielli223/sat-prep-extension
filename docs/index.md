...
| [`specs/2026-06-21-desmos-action-bar.md`](specs/2026-06-21-desmos-action-bar.md) | Decision (issue #29): move "Open real Desmos" onto the bottom action bar — picks up the deferred "move to bottom bar" item from #37. Compliant realization is **our** overlay's primary action row (`.fp-actions`), not CB's native footer (which `src/ui/` must not touch). Pure intra-overlay relocation; **reaffirms** the no-iframe / zero-license Desmos line. |
| [`plans/2026-06-21-desmos-action-bar.md`](plans/2026-06-21-desmos-action-bar.md) | Implementation plan for #29: failing structural test locking `.fp-desmos` into `.fp-actions` before `.fp-next`, then the one-line button move; `.fp-calc-pin` (Calculator) stays put. |
