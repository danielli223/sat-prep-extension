...
| [`specs/2026-06-21-seen-before-indicator.md`](specs/2026-06-21-seen-before-indicator.md) | Design (issue #28): show a done/missed/new "seen before" badge on the in-question overlay, derived purely from the student's own attempt journal (`getSeen` by question ID) — the list badger's compliant pattern extended into the question. Crosses no bright line: no new persisted field, fixed 3-value labels, no CB text. |
| [`plans/2026-06-21-seen-before-indicator.md`](plans/2026-06-21-seen-before-indicator.md) | Implementation plan for the seen-before indicator: `priorStatus` on `CardVM`/`toCardVM`, a `.fp-seen` badge in `answer-overlay.ts`, and a once-per-session `getSeen` snapshot threaded through `runLoop`/`showQuestion`. |
| [`specs/2026-06-21-reading-declutter-answer-top.md`](specs/2026-06-21-reading-declutter-answer-top.md) | Decision (issue #23): declutter the overlay so the answer stays near the top on Reading — hide the Math-only calculator on non-Math sections (`/math/i` on `CardVM.section`), collapse the "Why did you miss it?" note until a verdict exists, tighten spacing. Pure presentation change to our own Shadow-DOM overlay; crosses no bright line (note = student's own data, kept). |
| [`plans/2026-06-21-reading-declutter-answer-top.md`](plans/2026-06-21-reading-declutter-answer-top.md) | Implementation plan for the Reading declutter: test-first surface in `answer-overlay.test.ts`, the `answer-overlay.ts` changes (section-gated calc, collapsible note, tighter CSS), checker review, and the `/verify-overlay` visual-check note for the PR. |
...
| [`specs/2026-06-21-unify-calculator-desmos.md`](specs/2026-06-21-unify-calculator-desmos.md) | Decision (issue #17): unify the two calculators — **remove the GeoGebra embed**, collapse to a single "Calculator" button that opens the real Desmos externally. Resolves the prior spec's deferred "unify with Desmos" item the compliant way (no embed); **strengthens** the no-iframe / zero-license Desmos line. |
| [`plans/2026-06-21-unify-calculator-desmos.md`](plans/2026-06-21-unify-calculator-desmos.md) | Implementation plan for issue #17: locked test spec + maker steps to drop `toggleGeoGebra`/GeoGebra CSP, narrow the calculator telemetry enum to `'desmos'`, and render one "Calculator" button wired to `openDesmos()`. |
| [`specs/2026-06-21-inline-stats-widget-design.md`](specs/2026-06-21-inline-stats-widget-design.md) | Design (issue #16): replace the cryptic always-on "📓 Journal" pill with a compact top-right **at-a-glance stats widget** (done / accuracy% / day-streak from `deriveStats`) that auto-hides while a CB question modal is open and re-shows on the results list, still opening the full journal on click. Adds one fixture-tested CB signal `observeQuestionPresence`; no new persisted shape, no network, no AI — no bright line touched. |
| [`plans/2026-06-21-inline-stats-widget.md`](plans/2026-06-21-inline-stats-widget.md) | Implementation plan for the inline stats widget: TDD task-by-task — `observeQuestionPresence` in `cb/observer.ts`, `mountStatsWidget`/`updateStatsWidget`/`setStatsWidgetVisible` replacing `mountPanelToggle`, and the boot wiring. |
| [`specs/2026-06-21-question-grid-navigator-design.md`](specs/2026-06-21-question-grid-navigator-design.md) | Design (issue #25): bottom-of-page question-grid progress navigator — numbered cells over the **already-loaded** results, colored green/yellow/red by easy/medium/hard (color = difficulty only; state shown by glyph + label), click scrolls a loaded row into view. Persists nothing new; difficulty read live as taxonomy. Resolves the issue's two-color-axis conflict; no bright line crossed. |
| [`plans/2026-06-21-question-grid-navigator.md`](plans/2026-06-21-question-grid-navigator.md) | Implementation plan for the navigator: a taxonomy-only `difficulty` add to `list-reader.ts`, a new pure `ui/nav-grid.ts` (build + idempotent render), and the `content.ts` mount on the existing badger repaint triggers. TDD task-by-task. |
| [`specs/2026-06-22-nav-grid-collapsible-show-all-design.md`](specs/2026-06-22-nav-grid-collapsible-show-all-design.md) | Design (issue #76): follow-up to #25 — make the always-open bottom navigator **collapsed by default** behind a "Questions · N" toggle that expands the full grid of all **loaded** rows and collapses back, and change **cell-click to open the question** by clicking CB's own already-rendered `.id-column` row button (same posture as `clickCbNext`) instead of only scrolling. Picks up #25's explicitly-deferred "open the question"; "show all" stays scoped to loaded rows (no enumeration/prefetch, invariant #4). No new persisted data/network/AI. |
| [`plans/2026-06-22-nav-grid-collapsible-show-all.md`](plans/2026-06-22-nav-grid-collapsible-show-all.md) | Implementation plan for issue #76: TDD tasks — collapse/expand + `aria-expanded`/`aria-controls` + show-all tests in `nav-grid.test.ts`; an `openListQuestion(listRoot, id)` helper (click the row's `.id-column` button, fall back to scroll) in `resume.ts` with tests; the one-line `content.ts` `onJump` rewire. Human-gated `/verify-overlay` note (UI diff). |
| [`specs/2026-06-21-randomize-loaded-results-design.md`](specs/2026-06-21-randomize-loaded-results-design.md) | Design (issue #31, triage **NEEDS_REDESIGN**): the "Randomize (loaded results)" button only persisted a seed; nothing drove navigation by it. Fix = **guided shuffle** (the Resume model) — scroll the next shuffled row into view for the student to click; never auto-load or id-navigate (holds invariants #1 & #4). Bypasses CB's native Next in random mode only. |
| [`plans/2026-06-21-randomize-loaded-results.md`](plans/2026-06-21-randomize-loaded-results.md) | Implementation plan for issue #31: TDD tasks for the next-id helper (`shuffleIds`-composed) + `content.ts` wiring (pre-mint seed, guided start scroll, `onNext` random branch). List-mode navigation untouched; UI diff ⇒ `/verify-overlay` by the human reviewer. |
...
| [`specs/2026-07-05-per-question-timer.md`](specs/2026-07-05-per-question-timer.md) | Design + as-built: a live `.fp-timer` clock in the answer overlay showing time on the CURRENT question, reset (not per-id-cached) whenever a different question becomes active; the elapsed time at first Check is stamped onto `Attempt.timeSpentMs` (new optional field, `guard.ts` allowlist updated). No stats surfacing yet — raw storage + live display only. No bright line touched (student's own data, additive field). |
