...
import type { PriorStatus, SeenMap } from '../stats';
...
const LABEL: Record<PriorStatus, string> = { done: '✓ done', missed: '⚠ missed', new: 'new' };
...
const STYLE: Record<PriorStatus, string> = {
...
export function badge(listRoot: Element, seen: SeenMap): void {
  for (const { id, node } of readListQuestionIds(listRoot)) {
    ...
    const state: PriorStatus = seen[id] ?? 'new';
